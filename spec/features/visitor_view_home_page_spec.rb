require 'rails_helper'

feature 'Visitor view home page' do
  scenario 'successfully' do
    visit root_path
    
    expect(page).to have_content('Jobs | treina-dev02-etapa01')
  end 
    
  scenario 'Visitor do login' do
    user = User.create!(email:'test@test.com', password:'123456')
    login_as(user, :scope => :user)

    visit root_path

    expect(page).to have_link('Home')
    expect(page).to have_link('Sair')
    expect(page).to have_link('Perfil')
    expect(page).to have_link('Perfil candidato')
    expect(page).to have_link('Controle de acesso')
    expect(page).to have_content('BEM-VINDO ao Jobs')
    expect(page).to have_content('O maior site de empregos da América Latina')
  end
  
  scenario 'Visitor click_on perfil and see message error' do
    user = User.create!(email:'test@test.com', password:'123456')
    login_as(user, :scope => :user)

    visit root_path
    click_on('Perfil')
    
    expect(page).to have_content('Usuário não autorizado.')
  end

  scenario 'Visitor click_on perfil candidato and see message error' do
    user = User.create!(email:'test@test.com', password:'123456')
    login_as(user, :scope => :user)

    visit root_path
    click_on('Perfil candidato')
    
    expect(page).to have_content('Usuário não autorizado.')
  end

  scenario 'Visitor click_on controle de acesso and see message error' do
    user = User.create!(email:'test@test.com', password:'123456')
    login_as(user, :scope => :user)

    visit root_path
    click_on('Controle de acesso')
    
    expect(page).to have_content('Usuário não autorizado.')
  end

end
